...
namespace SemanticICP
{
    ...
    public:
        ...
        typedef std::vector<Eigen::Matrix<double, 6, 6>,
            ...
            CovarianceVector;
        typedef std::vector<Eigen::Matrix<double, 11, 1>,
        ...
        size_t N;
        EmIterativeClosestPoint(size_t n, int k = 20, double epsilon = 0.001) : kCorrespondences_(k),
                                                                                kEpsilon_(epsilon)
        {
            N = n;
        ...
        // EmIterativeClosestPoint(size_t n)
        // {
        //     N = n;
        // }
        ...
        inline void setConfusionMatrix(const Eigen::Matrix<double, 11, 11> &in)
        {
            confusion_matrix_ = in;
        }

        // void  align(PointCloudPtr finalCloud);
        void align(PointCloudPtr final_cloud, const Sophus::SE3d &init_transform);

        void ComputeCovariances(const PointCloudPtr cloudptr,
                                KdTreePtr treeptr,
                                MatricesVectorPtr matvec,
                                DistVectorPtr distvec);
        void getFusedLabels(PointCloudPtr labeledCloud, const Sophus::SE3d &transformation);
    ...
    protected:
        ...
        Eigen::Matrix<double, 11, 11> confusion_matrix_;
    };
